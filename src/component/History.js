...
const History = () => {
    ...
    const fetchData = async () => {
        const result = await axios.get(`http://localhost:8080/api/v1/coins/${params.symbol}/24h`);
        return result.data;
    }

    useEffect(() => {
        fetchData().then(data => setCoinHistory(data))
    }, []);

    const fetchHistoryData = async (e, timePeriod) => {
        e.preventDefault();
        const result = await axios.get(
            `http://localhost:8080/api/v1/coins/${params.symbol}/${timePeriod}`
        );
